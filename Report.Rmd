...
number is 1. For the `prodbf_iu` and `tbf` algorithms 5 or 6 times is counted and only the `prodbf_ic` seems to have a 
...
Finally, it the columns for `prodbf_iu` and `tbf_iu` are exactly equal. Lets check that out.
...
#### correlation metrics between algorithms
Also looking at the correlation between the algorithms, first for the dt with median BFs and afterwards for non-aggregated dt.
```{r, echo = F}
...
```
We see roughly the same trends. The only real difference seems that all correlations with `prodbf_ic` have lowered quite a bit. This is likely due to the algorithm being able to calculate very large estimates, while the other algorithms are more reserved.
...
#obtain numeric values conditions take on
