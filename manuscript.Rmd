...
Recent years have seen a crisis of confidence over the reliability of published results in psychology, and science more broadly [REF].
Replication research has come into focus as a solution to this crisis
and a way to derive knowledge that will stand the test of time.
In step with this interest in replication research,
research synthesis methods have blossomed.
These methods aggregate the findings of multiple studies,
either quantitatively [REF] or qualitatively [REF TMSR].
Between-studies heterogeneity is a key challenge in research synthesis.
Heterogeneity appears when studies examine the same research question in different laboratories, use idiosyncratic methods, and sample from distinct populations.

Meta-analysis is the most common type of research synthesis.
In meta-analysis, heterogeneity can be accounted for in four ways.
First, if studies are exact replications, one can assume that no heterogeneity exists and conduct a fixed-effect meta-analysis to estimate their common population effect.
Second, when differences between studies can be assumed to be random, random-effects meta-analysis can be used to estimate the mean of a distribution of population effects.
Third, when there are a few systematic differences between studies, these can be accounted for using meta-regression.
And finally, when there are many potential differences and it is not known beforehand which are relevant,
exploratory techniques like random forest meta-analysis and penalized metaregression can be used to identify relevant moderators.
Each of these approaches requires making different assumptions about the nature of heterogeneity.

An alternative approach that does not impose such assumptions is Bayesian evidence synthesis [BES, REF Kuiper 2013].
BES focuses on the aggregation of evidence in favor of an informative hypothesis $H_i$ across studies.
...
Note that other approaches to BES exist; for an overview, see REF Klugkist.

This tutorial paper introduces the first implementation of BES in userfriendly open source software.
A function `pbf()` was contributed to the `bain` R-package for Bayesian informative hypothesis evaluation, version XXX.
...
This correlation coefficient was either equal to or larger than an informative hypothesis value.
In this study, the informative hypothesis was kept constant, and specified as $H_i: \rho > .1$.
We then computed a PBF to determine the amount of evidence in favor of $H_i$ across these multiple samples.
...
The first was a naive "vote count" algorithm,
which used one-sided z-tests to examine whether a null hypothesis corresponding to the informative hypothesis was rejected in the majority of samples, i.e.: $H_0: \rho = .1$.
...
a multilevel model clustered by sample.
This algorithm, too, was evaluated using a 90% confidence interval for the overall effect size.
...
These quantities were summarized as sensitivity, $1-\frac{FN}{TP + FN}$, the probability of concluding the hypothesis is false given that it was truly false in the population,
and specificity, $1-\frac{FP}{FP + TN}$, the probability of concluding the hypothesis is true given that it was true in the population.
...
and the reliability of the two correlated variables, $\alpha \in (0.6, 0.8, 1.0)$, where 0.6 is the lowest reliability conventionally considered to be acceptable, and 1 represents an assumption of perfect reliability, as is the case when analyzing correlations between observed items or mean scale scores.
