...
# Tutorial
This tutorial about Bayesian Evidence Synthesis (BES) requires installation of `r R.Version()$version.string`, Rstudio version `r RStudio.Version()$version` and package `bain` version `r packageVersion('bain')`. This tutorial focusses on synthesizing evidence for an informative hypotheses using the Product Bayes Factor (pbf). To illustrate the method, a simulated counterpart of the data used in [REF Van Leeuwen et al. 2022] will be used avalable in `bain`. The data is used to investigate the relationship between political orientation and moral dispositions in three countries: the United states of America, Denmark and the Netherlands. A more detailed description of the dataset is either provided in [REF Van Leeuwen et al. 2022] or by running `?bain::synthetic_us`, `?bain::synthetic_dk` or `?bain::synthetic_nl`. 
...
In conventional Null-hypothesis significance testing (NHST), as the name implies, data is analysed to generate evidence for or against the null-hypothesis. This method can only evaluate a single hypothesis, the null-hypothesis, and its output is dichotomous in nature: whether a theoretical relationship is probable or improbable to exist given the data. However, there are many more hypotheses that can theoretically be tested and researchers may not be interested in the null-hypothesis. They may also not only be interested in whether the hypothesis is probable, but also by how much. In the Bayesian framework, the Bayes Factor (BF) is a test statistic that gives the amount of support for any possible desired hypothesis. Because the focus of this tutorial is on BES, this tutorial will only briefly introduce how to evaluate an hypothesis using the R package `bain` and interpret its output. For an extensive tutorial on how to evaluate informative hypotheses using `bain` see [@hoijtink2019tutorial].

Suppose we are interested in whether someone's socio-political orientation, i.e. liberal-conservative, is associated with the moral value of 'group loyalty'. Participants in all countries completed two scales that measure either socio-political orientation or group loyalty. Because each scale consists of multiple items of which some are reverse coded, single factors scores for both scales are first created for each country. Because the scales may differ per country, they also need to be standardized
...
Socio-political orientation and group loyalty have been aggregated to variables `soc` and `grp` respectively. Both are continuous variables with higher scores indicating conservatism or higher group loyalty respectively. In NHST we specify the null hypothesis that there is no association between `soc` and `grp`. We can test this hypothesis in each country by calculating a standardized regressoin coefficient.

$H_0: \beta_{grp} = 0$.

However, we may have prior expectations that higher group loyalty is somewhat related to  higher conservatism. An informative hypothesis could be:
...
We can test $H_i$ by running a linear regression on the data and passing the `lm` object with the specific hypotheses to `bain`. For the Netherlands, the Bayes Factor would be calculated as follows:
...
Sometimes there is access to the data of not only a single, but multiple studies that all contain the necessary information to evaluate our hypothesis $H_i$. To evaluate $H_i$, it is possible to synthesize the support for $H_i$ over all studies using the PBF calculated with `pbf()`. In this example, data to evaluate $H_i$ is available for three countries.

In the first use-case of `pbf()`, an `lm()` is ran for each country All `lm` objects are stored in a list which is passed to `pbf()`.
`pbf()` supports lists of all objects that `bain()` also supports. All supported types are specified in bain vignette *REF*. One exception is the use of named vectors, which will be covered later.
```{r}
...
The analyses have one hypotheses in common ($H_i: \beta_{grp} > .1$), but also contains one sample-specific hypothesis regarding $\beta_{her}$.
The `pbf()` function is called on the list of  resulting `bain` objects and returns output for the common hypotheses.
Note that, in this case, `pbf()` does not require an argument `hypotheses`, as the hypotheses are contained in the `bain` objects.

```{r}
bain_US <- bain(fit_US, "grp > .1; her = 0")
bain_DK <- bain(fit_DK, "grp > .1; her > 0")
bain_NL <- bain(fit_NL, "grp > .1; her < 0")
...
When there are no common hypotheses, `pbf()` throws an error.
...
In this case, one can use the default interface of `bain`, as explained in [REFERENCE BAIN PAPER Hoijtink, Mulder, van Lissa, & Gu, (2019) <doi:10.31234/osf.io/v3shc>].
...
### THESE DATA WILL BE USED (READ CASPARS WHATSAPP MESSAGES)
...
#   X = c(rnorm(1000,0,1), rnorm(1000, 0.2, 1)),
...
# y <- dat[dat$group == 1, 1]
# 
# ttest <- t_test(x,y, var.equal = TRUE, paired = FALSE)
# res <- bain(ttest, hypothesis = "y > x")
# 
# 
# fit_lm <- lm(X ~ group-1, data = dat)
# res_lm <- bain(fit_lm, hypothesis = "group1 > group0")
# 
# cor.test(y = dat$X, x = rep(0:1, each = 1000))
```
