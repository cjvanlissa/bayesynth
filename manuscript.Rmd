...
However, we may expect that the children growing up in an advantaged site will have a higher score than children growing up in a disdvantaged site. Our informative hypothesis is:

$H_A: \mu_{site2},\mu_{site3} > \mu_{site1},\mu_{site4},\mu_{site5}$
...
fit_bain <- bain(fit, hypothesis = "site1=site2=site3=site4=site5; site2&site3>site1&site4&site5" )
print(fit_bain)


```
<!-- Both BF.c and BF.u are close to zero, the probability of the HA being more probable than the Hc or Hu is small. <further interpretation>-->

### Single vs multiple studies.
In the case of multiple studies, it is possible to evaluate the hypotheses under consideration by synthesizing the evidence over all studies. In the example we split the `sesamesim` dataset up into three equal parts, to simulate three studies into the same subject.
In this example, each result has the $H_0$ and $H_A$ in common, but contains one result-specific hypothesis. The `pbf()` function only gives output for the hypotheses that are present in all results.
```{r}
anov <- lm(postnumb~site-1,sesamesim[1:75,])
results <- bain(anov, "site2=site1=site3=site4=site5; site2>site1=site3=site4=site5; site2&site3>site1&site4&site5")
anov2 <- lm(postnumb~site-1,sesamesim[76:150,])
results2 <- bain(anov, "site2=site1=site3=site4=site5; site2=site1=site3>site4=site5; site2&site3>site1&site4&site5")
anov3 <- lm(postnumb~site-1,sesamesim[151:nrow(sesamesim),])
results3 <- bain(anov, "site2=site1=site3=site4=site5; site2=site1=site3=site4>site5; site2&site3>site1&site4&site5")

pbf(list(results, results2, results3))

```
The function returns the individual BF.c's of each study, as well as their product.
