...
and a way to derive knowledge that will stand the test of time [see @lavelleWhenCrisisBecomes2021].
...
Both quantitative [e.g., @vanlissaSelectRelevantModerators2021] and qualitative [e.g., @vanlissaMappingPhenomenaRelevant2021] research synthesis methods exist.
The present paper focuses on quantitative methods.
...
Meta-analysis is the most common research synthesis method [@borensteinIntroductionMetaAnalysis2009].
...
An alternative approach that does not impose such assumptions is Bayesian evidence synthesis [BES, @kuiperCombiningStatisticalEvidence2013].
...
and the reliability of the two correlated variables, $\alpha \in (0.6, 0.8, 1.0)$, where 0.6 is the lowest reliability conventionally considered to be acceptable, and 1 represents perfect reliability, as is the assumed when analyzing correlations between observed items or scale scores.
