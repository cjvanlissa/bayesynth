...
<!-- maybe provide more examples of why hetergeneity can be an issue in meta-analysis. For example that the aggregation of model parameters is not as straightforward because they can be in different units (e.g. correlation vs odds ratios). while BF has no unit and can thus be easily aggregated. Caspar, do you agree that such examples may strengthen the paper? -->

Although each approach has its own specific assumptions, they all are regarding the nature of heterogeneity. 
...
Instead of aggregating parameter estimates of an effect over studies, which, due to heterogeneity, is not a straightforward endeavour, BES focuses on the aggregation of evidence in favor of an informative hypothesis $H_i$ across studies. The evidence in favor of $H_i$ is relative to some alternative hypothesis $H_a$.
...
The amount of evidence for this $H_i$ against an $H_a$ in each study and over all studies is expressed as a Bayes factor (BF). 
A Bayes factor can be interpreted as the ratio of evidence in favor of $H_i$ divided by evidence in favor of $H_a$. 
Thus, $BF_{ia}$ = 10 means that the data provide ten times more support in favor of $H_i$ than $H_a$. 
The BFs in each study are synthesized to one value by taking their product. 
This value is known as the Product Bayes Factor (PBF) and it summarizes the total evidence for the hypothesis against an alternative. 
...
In summary, BES is a three stage method where first both an $H_i$ and $H_a$ are formulated, second a $BF_{ia}$ is calculated in each study under consideration and, finally the product of all $BF_{ia}$'s is taken to obtain the PBF reflecting the overall support for $H_i$ over $H_a$. 
Do note that other approaches to BES exist [see "Bayesian Evidence Synthesis" in @heckReviewApplicationsBayes2022].

What also should be kept in mind is that meta-analysis and BES do answer different research questions.
Meta-analysis evaluates a certain theoretical relationship between variables. 
It pools parameter estimates that evaluate this relationship over multiple studies to obtain a combined effect. 
BES on the other hand compares a specific $H_i$ against some $H_a$.
Aggregating support for this $H_i$ against $H_a$ over studies shows how robust the evaluation is against heterogeneity.
The different answer from both methods thus make for complementary information. Meta-analysis provides support for or against a theoretical relationship in general, where BES provides support for a hypothesis about this theoretical relationship against some alternative.
Whether only one of the methods or both should be used depends on the goal of the evidence synthesis.
...
For each of the 108 x 1000 iterations of the simulation, we simulated a correlation coefficient, $\rho$, in multiple groups representing different studies examining a theoretical relationship.
$\rho$ was either equal to or .1 units larger than an informative hypothesis value.
...
The product was taken of the $BF_{ic}$ over all groups to compute the PBF and determine the amount of evidence in favor of $H_i$ over $H_c$ across the multiple groups
...
Other algorithms included a naive "vote count" algorithm, which used one-sided z-tests to examine whether a null hypothesis corresponding to the informative hypothesis was rejected in the majority of groups, i.e.: $H_0: \rho = .1$.
