...
<!-- Hier ook dan artikel van BEM noemen, maar ook zeggen dat het daar om null hypotheses gaat. -->
...
Even at a sample size of 500, the sensitivity of the other algorithms only rose slightly above .60, 
while for PBF this was .90.
This provides evidence that state-of-the-art evidence synthesis methods have little power to detect a true effect.
...
This is reflected in the accuracy of the algorithms at small sample sizes.
The accuracy at a sample size of 20, was around .50 for RMA, IPD and VC.
...
although it should be noted that even at a sample size of 500, PBFs specificity was .12 percentage points too low for conventional acceptable type I error rates.

With an increasing number of groups, slightly decreasing specificity was traded off for increasing sensitivity.<!-- I would argue that the decrease is negligable, 0.767 at k =2, 0.763 at k = 10 -->
...
Even with as much as ten available studies, PBF was the only algorithm that has a conventionally acceptable power.

With increasing reliability, increasing specificity was traded off for decreasing sensitivity.


There was no marginal condition in which the PBF had a conventional acceptable type I error rate.
Its mean was .76 which is almost .20 percentage points below .95.
However, the other algorithms had no marginal condition where an acceptable power was achieved.
The PBF can be used as a small sample size solution,
