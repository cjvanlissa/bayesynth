...
This was done by calculating the accuracy, sensitivity and specificity of the methods in different plausible varying conditions.
During the simulation the hypotheses remained constant $H_i: \rho > .1$, $H_0: \rho = .1$.
To be able to calculate both sensitivity and specificity, half of the simulated conditions contained an effect, $\rho = .2$, while the other half did not, $\rho = .1$.
...
The effect of sample size per study, number of studies and reliability of measurement on the performance of the algorithms was investigated.
...
With an increasing number of groups, specificity remained around .76 for PBF, while sensitivity increased to .87.
...
Looking at the specificity and sensitivity however, this is explained by the relatively low specificity of PBF and the very high specificity of RMA and IPD.
Thus,
...
As stated earlier, the PBF can be used as a small sample size solution,
...
The argument from Rouder and Morey (2011) is that we cannot take the PBF as a valid measure for meta-analysis. They show that calculating the BF in multiple samples and taking their product results in a different PBF than when all data is pooled and the BF is calculated after pooling. They also show another way of calculating a meta-analytic BF and that this pooled BF comes closer to that meta-analytic BF. However, this calculation assume a constant population effect and the same type of statistical model across studies
. where all data is drawn from a common distribution The idiosyncratic differences between studies makes that pooling data is not a straightforward endeavour, because of the heterogeneity of the effect. Rouder and Morey do acknowledge their choice for a constant effect to be debatable.
...
They also do it for a single parameter t, but it would not work if there are multiple different units.-->

The article of @rouder2011bayes states that a meta-analytic Bayes factor is not as straightforward as taking the product of $k$ individual Bayes factors.
...
Furthermore, as the article of @rouder2011bayes states itself, pooling data is valid if all data is independent and drawn from a common distribution.
...
In classical NHST, there is an equality resitriction. we either do or do not reject H0, so either rho = x or rho != x. Rejecting H0, means that the data suggests that rho != .1, but rather something else than .1 (so .2, .9, -.9 etc.)
With RMA and IPD we effectively say, rho = .1 or rho != .1 which is true or not (even if the test statistic = -.8,than .1 is not in the 90% CI and we still H0, even though its in the opposite direction.)
...
if rho = .1 and Rho = .1, than rejecting H0 is wrong. 
if rho = .2, and Rho = .1 than we must reject H0. -->
