...
# Tutorial
This tutorial about Bayesian Evidence Synthesis (BES) requires installation of `r R.Version()$version.string`, Rstudio version `r RStudio.Version()$version` and package `bain` version `r packageVersion('bain')`. 
This tutorial focusses on synthesizing evidence for an informative hypotheses using the Product Bayes Factor (pbf). 
To illustrate the method, a simulated counterpart of the data used in [@vanleeuwen2022morality] will be used available in `bain`. 
The data is used to investigate the relationship between political orientation and moral dispositions in three countries: the United states of America, Denmark and the Netherlands. 
...
Because the focus of this tutorial is on BES, this tutorial will only briefly introduce how to evaluate an hypothesis using the R package `bain` and interpret its output. 
For an extensive tutorial on how to evaluate informative hypotheses using `bain` see [@hoijtink2019tutorial].

Suppose we are interested in whether socio-political orientation, i.e. liberal-conservative, is associated with the subjective importance of the moral value 'group loyalty'. 
Participants in the three countries completed two scales of which one measured their socio-political orientation and the other their group loyalty. 
Because each scale consists of multiple items of which some are reverse coded, single factors scores for both scales are first created for each country. 
The scales also differ per country, and thus are standardized.
...
```{r}
# I create simple average scores for now
aggregate_scores <- function(country){
  df <- as.data.frame(cbind(
    scale(apply(country[,grepl("soc_", colnames(country))],1, mean)),
    apply(country[,grepl("grp_", colnames(country))],1, mean),
    apply(country[,grepl("her_", colnames(country))],1, mean)
    ))
  colnames(df) <- c('soc','grp', 'her')
  return(df)
  } 

NLa <- aggregate_scores(NL)
USa <- aggregate_scores(US)
DKa <- aggregate_scores(DK)
...
### Single vs multiple studies.
<!-- SESAMESIM DATA: Sometimes there is access to the data of not only a single, but multiple studies that all contain the necessary information to evaluate our hypothesis $H_i$. To evaluate $H_i$, it is possible to synthesize the support for $H_i$ over all studies using the PBF. In this example we split the `sesamesim` dataset up into three equal parts, to simulate three studies into the same topic.

```{r}
sample1 <- sesamesim[1:75,]
sample2 <- sesamesim[76:150,]
sample3 <- sesamesim[151:nrow(sesamesim),]
...
<!-- SESAME DATASET: The function returns a `data.frame` containing the passed hypotheses in the rows and in the columns the PBF and the individual Bayes Factor for each sample individually. The `PBF` of both hypotheses is close to zero, meaning that the combined evidence for both hypotheses over all studies is low. When looking at the individual Bayes Factors, we also see that the PBF is lower than all of the Bayes Factors individually. For sample 3 the specified hypotheses are even more likely than their complement. This example illustrates another strength of the PBF; being able to evaluate a hypothesis more extremely than the most extreme individual evaluation. If all samples do not support a hypothesis, than the number of times the hypothesis is not supported should be included in its evaluation. This leads to a more extreme combined evaluation than any individual evaluation. Likewise, if there is very strong support in only a small minority of the samples, while the majority of samples shows no support at all, than it is likely that the hypothesis is not true. -->
...
This makes it possible to, for example, evaluate different sets of hypotheses for multiple data sets before using the resulting `bain` objects to aggregate the evidence for all hypotheses those datasets share in common.
The example below illustrates this principle.
The analyses have one hypotheses in common ($H_i: \beta_{grp} > 0$), but also contains one sample-specific hypothesis regarding $\beta_{her}$.
The `pbf()` function is called on the list of the resulting `bain` objects and returns output for the common hypotheses.
Note that, in this case, `pbf()` does not require an argument `hypothesiss`, as the hypotheses are contained in the `bain` objects.

```{r}
bain_US <- bain(fit_US, "grp > 0; her = 0")
bain_DK <- bain(fit_DK, "grp > 0; her > 0")
bain_NL <- bain(fit_NL, "grp > 0; her < 0")

bain_all <- list(bain_US, bain_DK, bain_NL)

pbf(bain_all)

```

As can be seen, the results are identical to the results in the previous example. This means that the sample-specific hypotheses have been left out and only the hypotheses that each sample has in common are retained.
When there are no hypotheses at all that are common in all objects, `pbf()` throws an error.
...
These arguments may be derived in different ways for distinct samples.
...
In this specific use-case, let's suppose three different statistics have been calculated to analyse the data of the three countries. 
Each statistic was calculated to establish the relationship between `grp` and `soc`:
1. A correlation coefficient was calculated using the US data.
2. A standardized regression coefficient for `grp` was calculated using the DK data.
3. A t-test was performed using the NL data. 
To simulate groups to perform a t-test on, the `grp` variable in the NL data will be recoded to 0 and 1 which represent a score below or above the median of `grp` respectively.

The current $H_i: \beta_{grp} > 0$ effectively tests the following:
*There is a positive association between political conservatism and subjective importance of group loyalty*
But $\beta_{grp}$ is a regression-specific parameter which was not calculated in two out of three studies.
Therefore, the three sample-specific hypotheses that answer the same underlying $H_i$ are:
- 1. $H_i^{US}: \rho_{grp,soc} > 0$
- 2. $H_i^{DK}: \beta_{grp} > 0$
- 3. $H_i^{NL}: \mu_{soc}^{conservative} > \mu_{soc}^{liberal}$


These statistics to test the hypotheses along with their standard error can be passed to `bain` and do not require the raw data. 
The pbf can then be calculated and a single score can be synthesized to evaluate $H_i$. 
This despite different statistical methods being used to establish the relationship between `grp` and `soc`.

```{r}
US_est <- c(grp = cor(USa$soc, USa$grp))
US_se <- sqrt(1-US_est^2) / sqrt(nrow(USa)-2) # formula for SE of correlation coefficient: sqrt(1-r^2)/sqrt(n-2)

DK_fit <- summary(lm(soc~grp, data = DKa))
DK_est <- c(grp = DK_fit$coefficients['grp', 'Estimate'])
DK_se <- DK_fit$coefficients['grp', 'Std. Error']

### EB: I have a question When calculating the estimate for NL. I assume I need the effect size of the t-test, rather than the t-statistic itself. This requires Cohen's d. Is this correct? In this case I do not even need to do a t-test, I only need the descriptive statistics of the two samples

NLa$grp_recoded <- as.factor(sapply(NLa$grp,function(i){ ifelse(i < median(NLa$grp), 0, 1) }))
liberals <- NLa$soc[NLa$grp_recoded == 0]
conservatives <- NLa$soc[NLa$grp_recoded == 1]

nlib <- length(liberals)
ncon <- length(conservatives)
pooled_sd <- sqrt(((ncon-1)*sd(conservatives) + (nlib-1)*sd(liberals)) / (nlib+ncon-2) )

NL_est <- c(grp = (mean(conservatives) - mean(liberals)) / pooled_sd)
NL_se <- sqrt(((ncon+nlib)/(ncon*nlib)) + NL_est^2/(2*(ncon+nlib))) # formula for SE of cohen's D retrieved from https://stats.stackexchange.com/questions/495015/what-is-the-formula-for-the-standard-error-of-cohens-d#:~:text=Cohen's%20d%20is%20defined%20as,%E2%88%922%20(see%20here).

```
The values are then converted to `bain` objects, put in a list, and passed to `pbf()`. 
```{r}
US_bain <- bain(x = US_est, Sigma = matrix(US_se, 1, 1), n = nrow(USa), hypothesis = hypotheses)
DK_bain <- bain(x = DK_est, Sigma = matrix(DK_se, 1, 1), n = nrow(DKa), hypothesis = hypotheses)
NL_bain <- bain(x = NL_est, Sigma = matrix(NL_se, 1, 1), n = nrow(NLa), hypothesis = hypotheses)

pbf(list(US_bain, DK_bain, NL_bain))
```

The output provides evidence for or against the hypothesis: *there is a positive association between political conservatism and subjective importance of group loyalty*.
Although each study used a different method to operationalize the evidence for that hypothesis, their evidence can be synthesized to a single value using the pbf.
